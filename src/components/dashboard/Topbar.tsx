...
export default function Topbar({ onMenuClick }: TopbarProps) {
  ...
  const getLogIcon = (action: string) => {
    const act = action || '';
    if (act.includes('PAYMENT') || act.includes('CLEAR')) return <CheckCircle2 size={16} className="text-status-success shrink-0 mt-0.5" />;
    if (act.includes('ERROR') || act.includes('DISTRESS') || act.includes('FAIL')) return <AlertCircle size={16} className="text-status-danger shrink-0 mt-0.5" />;
                ...
                {latestLogs.map((log) => (
                  <div key={log.id} className="flex gap-3 hover:bg-surface-bg/50 p-2 -mx-2 rounded-xl transition-colors">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="text-[9px] text-text-muted">{formatDate(log.timestamp || new Date().toISOString())}</div>
                    </div>
                  </div>
                ))}
